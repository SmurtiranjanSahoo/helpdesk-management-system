...
import BestPracticsText from "../../components/BestPracticsText";
import AnswerFormatter from "../../components/AnswerFormatter";

const defaultPromptsSheets = [
  `Extract the fourth word in A1 only when cell B1 is greater than 100 and when C1 starts with the word "hello" and ends with the word "goodbye" in Google Sheets.`,
  "Use a formula to extract the first, middle, and last name from a cell that contains a person's full name in Google Sheets.",
  `Sum of column A when column B contains the word "marketing" and column C is today's date in Google Sheets.`,
  "Use a formula to calculate the compound annual growth rate (CAGR) for a series of values over a given period of time in Google Sheets.",
];
const defaultPromptsExcel = [
  `Extract the fourth word in A1 only when cell B1 is greater than 100 and when C1 starts with the word "hello" and ends with the word "goodbye" in MS Excel.`,
  "Use a formula to extract the first, middle, and last name from a cell that contains a person's full  in MS Excel.",
  `Sum of column A when column B contains the word "marketing" and column C is today's  in MS Excel.`,
  "Use a formula to calculate the compound annual growth rate (CAGR) for a series of values over a given period of time in MS Excel.",
];
...
const Formula = () => {
  ...
  const [defaultPrompts, setDefaultPrompts] = useState(defaultPromptsSheets);
  ...
  const handleSubmitRequest = async (type) => {
    ...
    let { data } = await axios.post(`${BASE_URL}/ans/formula`, {
      ...
      <div className="xs:w-1/2 xs:mr-2 bg-gray-50 xs:border p-2 xs:p-4 xs:rounded-lg">
        ...
        <div className="mb-4">
          ...
          <div className="flex mt-2">
            <div
              onClick={() => {
                setDefaultPrompts(defaultPromptsExcel);
                setSearch((prev) => ({
                  ...prev,
                  currentOptions: {
                    ...prev.currentOptions,
            ...
            <div
              onClick={() => {
                setDefaultPrompts(defaultPromptsSheets);
                setSearch((prev) => ({
                  ...prev,
                  currentOptions: {
                    ...prev.currentOptions,
        ...
        <div className="hidden xs:block mb-6">
          <p className="text-xs mb-1">
            What is the formula you want to generate?
          </p>
          <div className="flex flex-col ">
            ...
            <div className="flex py-2">
              {/* <div className="text-xs text-blue-600 cursor-pointer flex ">
                Best Practices{" "}
                <HiOutlineExternalLink className="text-lg ml-2" />
              </div> */}
